...
for turn in range(m):
    ...
    if board[ry][rx] > 0:  # 충돌시
        ...
        while len(nextSanta) > 0:
            if nextSanta[y] <= 0 or nextSanta[y] > n or nextSanta[x] <= 0 or nextSanta[x] > n:  # out
                ...
                santa[nextSanta[index]] = [nextSanta[y], nextSanta[x],
                                           nextSanta[state], nextSanta[index]]  # 산타에 저장
                nextSanta = []
            else:  # in board
                if board[nextSanta[y]][nextSanta[x]] > 0:  # 그 자리에 누구 있음
                    nextIndex = board[nextSanta[y]][nextSanta[x]]  # 튕겨져 나갈 놈
                    board[nextSanta[y]][nextSanta[x]
                                        ] = nextSanta[index]  # 그 자리에 놓음
                    santa[nextSanta[index]] = [nextSanta[y], nextSanta[x],
                                               nextSanta[state], nextSanta[index]]  # 산타에 저장
                    nextSanta = [santa[nextIndex][y]+dy, santa[nextIndex]
                                 [x]+dx, santa[nextIndex][state], nextIndex]
                else:  # 아무도 없음
                    board[nextSanta[y]][nextSanta[x]
                                        ] = nextSanta[index]  # 그 자리에 놓음
                    santa[nextSanta[index]] = [nextSanta[y], nextSanta[x],
    ...
    for i in range(1, p+1):
        ...
        if nextSanta[y] == ry and nextSanta[x] == rx:  # 루돌프 충돌
            ...
            nextSanta = [nextSanta[y] - closeDirc[y] * d, nextSanta[x] -
                         closeDirc[x] * d, 1, nextSanta[index]]  # 튕기는 위치

            while len(nextSanta) > 0:
                if nextSanta[y] <= 0 or nextSanta[y] > n or nextSanta[x] <= 0 or nextSanta[x] > n:  # out
                    santa[nextSanta[index]] = [nextSanta[y],
                                               nextSanta[x], -1, nextSanta[index]]  # 산타에 저장
                    nextSanta = []
                else:  # in board
                    if board[nextSanta[y]][nextSanta[x]] > 0:  # 그 자리에 누구 있음
                        nextIndex = board[nextSanta[y]
                                          ][nextSanta[x]]  # 튕겨져 나갈 놈
                        board[nextSanta[y]][nextSanta[x]
                                            ] = nextSanta[index]  # 그 자리에 놓음
                        santa[nextSanta[index]] = [nextSanta[y], nextSanta[x],
                                                   nextSanta[state], nextSanta[index]]  # 산타에 저장
                        nextSanta = [nextSanta[y]-closeDirc[y], nextSanta[x] -
                                     closeDirc[x], santa[nextIndex][state], nextIndex]
                    else:  # 아무도 없음
                        board[nextSanta[y]][nextSanta[x]
                                            ] = nextSanta[index]  # 그 자리에 놓음
                        santa[nextSanta[index]] = [nextSanta[y], nextSanta[x],
